         ...
         switch(type) {
            ...
            case SC_EXEC: {
               ...
               case SC_JOIN: {
                  ...
                  // Get the param in the registers
                  int addr_debugName = g_machine->ReadIntRegister(4);
                  int count = g_machine->ReadIntRegister(5);

                  //Build the name of the semaphore
                  int size = GetLengthParam(addr_debugName);
                  char sem_name[size];
                  GetStringParam(addr_debugName, sem_name, size);

                  ptSemaphore = new Semaphore(sem_name,count);
                  int32_t tid;
                  tid = g_object_ids->AddObject(ptSemaphore);
                  ...
                  case SC_SEM_DESTROY:{
                     ...
                     SemId sem_id = (SemId) g_machine->ReadIntRegister(4);
                     Semaphore * pt_sem = (Semaphore * ) g_object_ids->SearchObject(sem_id);

                     if(pt_sem->type == SEMAPHORE_TYPE){
                  ...
                  case SC_V:{
                     ...
                     SemId sem_id = (SemId) g_machine->ReadIntRegister(4);
                     Semaphore* ptr_sem = (Semaphore*) g_object_ids->SearchObject(sem_id);

                     if(ptr_sem->type == SEMAPHORE_TYPE){
                  ...
                  case SC_LOCK_DESTROY:{
                     LockId lock_id = (LockId) g_machine->ReadIntRegister(4);
                  ...
                  case SC_LOCK_ACQUIRE:{
                     LockId lock_id = (LockId) g_machine->ReadIntRegister(4);
                     Lock* ptr_lock = (Lock*)g_object_ids->SearchObject(lock_id);

                     if(ptr_lock->type == LOCK_TYPE){
                        ptr_lock->Acquire();
                        g_machine->WriteIntRegister(2,0);
                        g_syscall_error->SetMsg((char*)"",NO_ERROR);
                     }else{
                  ...
                  case SC_COND_CREATE:{
                    ...
                    Condition* ptr_cond;
                    int addr_name = g_machine->ReadIntRegister(4);

                    //Build the name of the semaphore
                    int size = GetLengthParam(addr_name);
                    char sem_name[size];
                    GetStringParam(addr_name, sem_name, size);

                    ptr_cond = new Condition(sem_name);
                    int32_t tid;
                    tid = g_object_ids->AddObject(ptr_cond);
                  ...
                  case SC_COND_DESTROY:{
                     ...
                     CondId condId = (CondId) g_machine->ReadIntRegister(4);
                     Condition* ptr_cond = (Condition*)g_object_ids->SearchObject(condId);

                     if(ptr_cond->type == CONDITION_TYPE){
